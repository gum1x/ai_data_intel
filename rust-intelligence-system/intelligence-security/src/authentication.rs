...
impl AuthenticationService {
    pub fn new(jwt_secret: String, jwt_expiry: Duration) -> Self {
        Self {
            jwt_secret,
            jwt_expiry,
            refresh_token_expiry: Duration::from_secs(7 * 24 * 3600), // 7 days
    ...
    fn create_session(
        ...
        let session = Session {
            id: session_id,
            user_id,
            access_token_hash: String::new(), // Will be set when token is generated
            refresh_token_hash: String::new(), // Will be set when token is generated
    ...
    fn find_session_by_refresh_token(&self, refresh_token: &str) -> IntelligenceResult<Uuid> {
        // In a real implementation, you would hash the refresh token and store it
        // For simplicity, we'll use the token directly as session ID
